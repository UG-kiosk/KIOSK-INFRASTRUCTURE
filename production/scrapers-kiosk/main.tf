...
locals {
  docker_image = "${module.shared_envs_prod.docker_username}/kiosk-scrapers-prod:${var.docker_image_tag}"
}
...
resource "azurerm_private_endpoint" "scrapers_kiosk_private_endpoint" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_linux_web_app.scrapers-kiosk.id
    subresource_names              = ["sites"]
  }
}

resource "azurerm_private_dns_a_record" "scrapers_kiosk_private_dns_a_record" {
  name                = azurerm_linux_web_app.scrapers-kiosk.name
  zone_name           = data.terraform_remote_state.kiosk_private_dns_state.outputs.private_dns_zone_name
  resource_group_name = module.shared_envs_prod.resource_group_name
  ttl                 = 300
  records             = [azurerm_private_endpoint.scrapers_kiosk_private_endpoint.private_service_connection[0].private_ip_address]
}
